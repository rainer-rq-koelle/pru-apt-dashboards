...
```{r rwy-config, comment=NA}
params$config %>%
  select(CONFIGURATION, SHARE_PCT) %>%
  mutate( SHARE_PCT = round(100 * SHARE_PCT)
         ,SHARE_PCT = paste0(SHARE_PCT, "%")) %>%
  knitr::kable( "latex" #, booktabs = TRUE,
    ,col.names = c("runway configurations in 2019", "")
    )
#%>%
...
#  mutate( SHARE_STR = round(100 * SHARE_PCT)
#         ,STRING    = paste(CONFIGURATION, SHARE_STR, sep = "   ") 
#         ,STRING    = paste("* ", STRING, "%")) %>% 
#  pull(STRING)
#writeLines(config)
```

>Only configurations with a minimum of 3% 
>share are shown.
...
```{r}
p <- ggplot(mtcars[1:10, c('mpg', 'disp')]) +
  geom_point(aes(x = mpg, y = disp))

#patchwork   
patch <- p + gridExtra::tableGrob(mtcars[1:10, c('mpg', 'disp')])
patch
```
...
g1 <- ggplot(data = mvts_pa, mapping = aes(x = YEAR, y = FLT_TOT, fill = YEAR
                                           #, colour = YTD
                                           #, size = YTD   - not clear when used and throws warning
                                           )
             ) +
  geom_col(width = 0.8) + 
 ...
 # scale_size_manual(values = c(NA, 2)) +
  ...
  labs(x = "", y = "total IFR movements")

# monthly trend
mvts_pm <- params$tfc %>% select(YEAR, MONTH_MON, MONTH_NUM, FLT_TOT_1) %>%
  group_by(YEAR, MONTH_MON, MONTH_NUM) %>%
  summarise(FLT_TOT = sum(FLT_TOT_1, na.rm = TRUE), .groups = "drop") %>% ungroup() %>%
  arrange(factor(MONTH_NUM, levels=month_num_c))

## year-to-date traffic growth
grwth_pm <- mvts_pm %>% filter(YEAR == max(YEAR))
y0 <- grwth_pm$FLT_TOT[1]  # beginning of year, i.e. Febr ~ 28 days = small dip
yn <- grwth_pm$FLT_TOT[  nrow(grwth_pm)] # current month flights
mth<- grwth_pm$MONTH_NUM[nrow(grwth_pm)] # current month name
mth_int <- sub("^0+", "",mth) # truncate leading zero
mth_int <- as.integer(mth_int) %>% max()
month_lbl <- month_lbls[mth_int] # current (or max supplied) month label
grwth_ann <- paste0("year-to-date \n growth (", month_lbl,"): ", round(100*((yn-y0)/y0), 1),"%")  

g2 <- ggplot(data = mvts_pm
             , mapping = aes(x = MONTH_NUM, y = FLT_TOT, group= YEAR, colour = YEAR)) +
  geom_line() + 
  expand_limits(y = 0) + 
  # MONTH_NUM is a CHARACTER !!!!
  ...
  theme( legend.position = c(0.95, 0.05), legend.justification = c("right", "bottom")
       ...
       ,panel.grid.major.x = element_blank()) +
  labs(x = "", y = "") + 
  annotate("text", x ="02", y = 0.5 * y0, label = grwth_ann, size = 5, hjust=0)
```
...
g1 <- ggplot(data = atfm_pa %>% filter(YEAR != lubridate::year(today()))
  ...
  theme(legend.position = "none"
       ...
       ,panel.grid.major.x = element_blank()
       ,panel.grid.minor.x = element_blank()) +
  labs(x = "", y = "average arrival ATFM delay [min/arrival]")
...
g1 <- ggplot(data = asma_pa, mapping = aes(x = YEAR, y = SHARE, fill = TIME)) +
  geom_col(width = 0.8) +
  theme_minimal() + scale_y_continuous(labels = scales::comma) +
...
g1 <- ggplot(data = txot_pa, mapping = aes(x = YEAR, y = SHARE, fill = TIME)) +
  geom_col(width = 0.8) +
  theme_minimal() + 
...
pddly_pm <- params$pddly %>% filter(YEAR == max(YEAR)) %>%
  ...
  select(YEAR, MONTH_NUM, FLT_DEP, TOT_DLY_89, TOT_DLY_OTHER, TOT_DLY_UNID) 
...
ytd <- pddly_pm %>% 
  summarise( MONTH      = max(MONTH_NUM)
            ,FLT_DEP    = sum(FLT_DEP)
            ,TOT_DLY_89 = sum(TOT_DLY_89), TOT_DLY_OTHER = sum(TOT_DLY_OTHER)
            ,TOT_DLY_89_UNID = sum(TOT_DLY_UNID)
            ,MAX_TOTAL  = TOT_DLY_89 + TOT_DLY_OTHER + TOT_DLY_UNID
            )
msg <- paste0("year-to-date (", month_lbls[ytd$MONTH] ,") average" 
              ,"\nATC pre-departure delay: "
              , round(ytd$TOT_DLY_89 / ytd$FLT_DEP, 2), " min/dep.")
...
Further performance related data can be downloaded from the performance portal at https://ansperformance.eu. Browse for the data section.
