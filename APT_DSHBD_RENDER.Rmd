...
pick_mth <- c("Jan", "Feb", "Mar", "Apr", "May", "Jun",
              "Jul", "Aug", "Sep", "Oct", "Nov", "Dec")
...
if( file.exists(apt_layout_file)) {
  knitr::include_graphics(apt_layout_file)
}
```
...
## Row -----------------------------------------------------------
### Annual Punctuality
```{r Punctuality YEARLY LAYOUT}

 
```

### Monthly Punctuality
```{r Punctuality MONHTLY LAYOUT}
...
cat("
Analysis on approach times are only available for airports providing the required data 
...
cat("
Analysis on taxi times are only available for airports providing the required data 
using the Airport Operator Data Flow (APDF).
...
cat("
Analysis on pre-departure delay are only available for airports providing the required data 
using the Airport Operator Data Flow (APDF).
...
cat("
Analysis on turnaround times are only available for airports providing the required data 
using the Airport Operator Data Flow (APDF).
...
TURN_MM %>%  plot_ly() %>% 
              ...
              layout( updatemenus = list( button_type_list ),
                      hovermode   = "x unified",
                      xaxis       = tick_yr_no_title,
                      yaxis       = list(title = "Average Turnaround times [min]",
                                         titlefont = list(size = 11))) %>%
              config( displaylogo = FALSE,
                      modeBarButtonsToRemove = config_bar_remove_buttons)
}
