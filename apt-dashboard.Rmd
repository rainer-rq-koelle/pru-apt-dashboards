...
#   name:   !r cucu$name
#   state:  !r cucu$state
#   config: !r cucu$config
#   ldgsum: !r cucu$ldgsum
#   latest: !r cucu$latest
#   covid:  !r cucu$covid
#   tfc:    !r cucu$tfc
#   thru:   !r cucu$thru
#   atfm:   !r cucu$atfm
...
output: 
  ...
  #  mathjax: NULL          # no math symbols/equations used
    self_contained: FALSE # FALSE  # do not include plotly libs in html output
---
...
```{r airport-layout}
fs::file_copy(paste0("data-ad-rwy-charts/", params$icao, ".png"),
              paste0("docs/", params$icao, ".png"), overwrite = TRUE)
...
### COVID19 Snapshot Traffic Evolution (movements and percentage change over time vs 2019)
...
fig1 <- params$covid %>%
  plot_ly() %>%
  add_lines(x=~DAY, y=~FLTS_2020, name = "Movements") %>%
...
mvts_pm %>%
  ...
  add_trace( type = 'scatter', mode = 'markers+lines'
             ,hovertemplate = "%{y:.r}"     # define hoverformtting for y, rounded with no significant digit
             ) %>%
...
g1 <- pddly_pa %>%
  plot_ly(x=~YEAR, y=~DLY_DUR, type="bar", color=~DLY_CAT, legendgroup=~DLY_CAT
          # control digits/scientific notation in hover: .r = rounded no digit
          ,hovertemplate = "%{y:.r}"
          ) %>%
  layout( barmode = "stack"
         ,hovermode = "x unified" 
