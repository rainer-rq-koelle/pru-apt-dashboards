...
#` For RELEASE 1 IT WAS DECIDED TO SIMPLIFY THE SUMMARY TABLE
#' I have no idea what I had been doing to implement the spartklines
#' Thus, we "OVERWRITE" the info here ... and I promise to spend some
#' time on relearning what I hacked together below`
...
prepare_front_page_DT <- function(.icao, .tfc, .asma, .txot, .atfm
  ...
  IND <- c("TFC", "ASMA","TXOT","ATFM_A")
  NAM <- c( "Total traffic (annual & recent month)"
           ,"Average additional ASMA time [min/arr]"
           ,"Average additional taxi-out time [min/dep]"
           ,"Average arrival ATFM delay [min/arr]"
  )
  nam <- tibble(IND = IND, NAM = NAM) #???? what is this?


  # binding the rows for the desired output table
  ...
  # add ldgsummaries --------------------- RELEASE 1 FIX --------------
  ...
  out$CHANGE_M2M_P<- NULL # delete monthly change column

  # add latest indicators measured and format 2 digits
  out <- out %>% mutate(CURRENT = "0")   # overwrite current
