                                  ...
                                  .latest)
{
  ...
  calc_current_month <- function(.perf)
    {
    df <- .perf %>%
            mutate(DIFF_FLT      = FLT_TOT - lead(FLT_TOT),
            ...
            select(YEAR, MONTH_NUM, MOF, FLT_TOT, DIFF_FLT, DIFF_FLT_P, 
  ...
  spark_tbl <- tibble(IND = c("TFC"  ,"ASMA"   ,"TXOT"   ,"ATFM_A"),
                      SPK = c(tfc_spk, asma_spk, txot_spk, atfm_spk))

  r_tfc <- perf2 %>% filter(CHECK == "NOW") %>% 
            mutate(IND = "TFC") %>%
            select(IND, CURRENT = FLT_TOT, CHANGE_M2M_P = DIFF_FLT_P)

  r_asma<- perf2 %>% filter(CHECK == "NOW") %>% 
            mutate(IND = "ASMA") %>%
                                        ...
                                        .latest)
{
  ...
  spark_tbl <- tibble(IND = c("TFC"  ,"ATFM_A"),
