...
library(readr)     # Loading library [readr]     ----
library(DBI)       # Loading library [DBI]       ----
library(ROracle)   # Loading library [ROracle]   ----
#.----
...
EXPORT_QUERY <- function(schema, query)
{
  ...
  con <- withr::local_db_connection(DBI::dbConnect(DBI::dbDriver("Oracle"),USR, PWD, dbname = DBN,timezone = "UTC"))
  con %>%
    dbSendQuery(query) %>%
    fetch(n = -1)
}
...
{
  QUERY <- "SELECT * FROM PRU_AIRPORT.STAT_NM_APT_THROUGHPUT WHERE AIRPORT IN (SELECT DISTINCT AIRPORT FROM PRU_AIRPORT.V_STAT_TOP100_APT_INFO)"
  EXPORT_QUERY("PRU_AIRPORT", QUERY) 
}
