...
APT_DF      <- readr::read_csv2("./data/PRU_AIRPORT_INFO.csv")
APT_DF      <- APT_DF%>%
...
CONFIG_DF   <- readr::read_csv2("./data/APDF_RWY_CONFIGURATION.csv")
CONFIG_DF   <- CONFIG_DF %>%
                na.omit() %>%
...
#..NM APT TRAFFIC ----
TFC_DF    <-readr::read_csv2("./data/NM_APT_TRAFFIC.csv")

#..NM APT TRAFFIC VARIATION ----

TFC_VAR_DF <- readxl::read_excel(
  path  = here::here("data", "NM_APT_TRAFFIC_MOV_AVG.xlsx"),
  sheet = "Sheet 1",
  range = cell_cols("A:F")
)
...
#..NM APT THROUGHPUT ----
THRU_DF   <- readr::read_csv2("./data/NM_APT_THROUGHPUT.csv") %>% 
...
#..PIP ATFM DELAY ----
ATFM_DF  <- readxl::read_excel("./data/PIP_Airport_Arrival_ATFM_Delay.xlsx", sheet = "DATA") %>%
...
#..PIP SLOT ADHERENCE ----
SLOT_DF  <- readxl::read_excel("./data/PIP_ATFM_Slot_Adherence.xlsx", sheet = "DATA")


# .................----
# APDF MONTLHY DATA ----
APDF_MM_DF <- readr::read_csv2("./data/APDF_MONTHLY_DATA.csv")
# .................----
...
ASMA_YY_DF <- APDF_MM_DF %>%
                ...
                summarise( TOT_UNIMP_TIME = sum(ASMA_REF_TIME_MIN, na.rm = TRUE),
...
TXOT_YY_DF <- APDF_MM_DF %>%
                ...
                group_by(AIRPORT, APT_ICAO, YEAR) %>%
                summarise( TOT_UNIMP_TIME = sum(TAXI_OUT_REF_TIME_MIN, na.rm = TRUE),
...
TXIN_YY_DF <- APDF_MM_DF %>%
                ...
                group_by(AIRPORT, APT_ICAO, YEAR) %>%
                summarise( TOT_UNIMP_TIME = sum(TAXI_IN_REF_TIME_MIN, na.rm = TRUE),
...
PDDLY_YY_DF <- APDF_MM_DF %>%
                ...
                group_by(AIRPORT, APT_ICAO, YEAR) %>%
                summarise( TOT_FLT_DEP        = sum(NB_DLY_DEP_FL,    na.rm = TRUE),
                ...
                ungroup%>%
                mutate(TOT_DLY_UNID  = TOT_DLY_999 + TOT_DLY_ZZZ + TOT_DLY_UNREPORTED,
...
APDF_TURN_DF <- readr::read_csv2("./data/APDF_TURNAROUND_DATA.csv")
# .................----
